...
*/
    ...
    httpRequest.open('GET', 'jsonfeed_test.json', true);	
    httpRequest.send(null);

    httpRequest.onreadystatechange = function(){
      if(httpRequest.readyState === 4){
        var statusType = (httpRequest.status / 100) | 0;
		feed = JSON.parse(httpRequest.responseText);
...
app.dpi.module('animation', ['feed', 'documentLoad', 'fonts',
    function (feed) {
        ...
        tween1.to(ball, 0.6, {top: 80, right: 70})
            .to(ball, 0.6, {top: 90, right: 100}, '-=0.6')
            .to(ball, 0.6, {top: 150, right: 110, rotation: -100}, '-=0.2')
            .to(ball, 0.6, {top: 420, right: 60})
            .to(ball, 0.6, {top: 170}, '-=0.2')
            .to(icon, 1.2, {opacity: 1}, '-=1.2')
            .to(ball, 0.6, {top: 420}, '-=0.2')
            .to(ball, 0.6, {top: 540, right: 160, rotation: 360}, '+=0.2')	
